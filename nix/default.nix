  ...
  testConfigurations =
    let
      makeTestConfiguration = { ghcVersion, overrides ? new: old: { } }:
        ...
        (pkgs.haskell.packages.${ghcVersion}.override (old: {
          overrides =
            combineOverrides old [
              (packageSourceOverrides { partial-semigroup-hedgehog = ../partial-semigroup-hedgehog; })
              overrides
            ];
    ...
    rec {
      ...
      ghc-9-6 = makeTestConfiguration {
        ghcVersion = "ghc96";
        overrides = new: old: {
          partial-semigroup = new.callPackage ./haskell/partial-semigroup.nix { };
        };
      };
      all = pkgs.symlinkJoin {
        name = "partial-semigroup-hedgehog-tests";
